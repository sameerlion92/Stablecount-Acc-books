...
export async function prepareDatabase() {
  ...
  const bankColumns=await db.prepare("PRAGMA table_info(bank_accounts)").all();
  const bankColumnNames=new Set(bankColumns.results.map(column=>String((column as Record<string,unknown>).name)));
  const bankAdditions=["account_number TEXT NOT NULL DEFAULT ''","account_name TEXT NOT NULL DEFAULT ''","bank_address TEXT NOT NULL DEFAULT ''","swift_code TEXT NOT NULL DEFAULT ''","ifsc_code TEXT NOT NULL DEFAULT ''","account_type TEXT NOT NULL DEFAULT ''"];
  for(const addition of bankAdditions){const name=addition.split(" ")[0];if(!bankColumnNames.has(name))await db.prepare(`ALTER TABLE bank_accounts ADD COLUMN ${addition}`).run();}
...
export async function POST(request: Request) {
  try {
    ...
    if(payload.action==="settings"){
      ...
      const accountNumber=String(required(payload,"accountNumber")).replace(/\s+/g,"");
      if(!accountNumber)throw new Error("Full account number is required");
      const accountLast4=accountNumber.slice(-4);
      const bank=await db.prepare("INSERT INTO bank_accounts (nickname,bank_name,currency,account_last4,account_number,account_name,bank_address,swift_code,ifsc_code,account_type,balance,is_default) VALUES (?,?,?,?,?,?,?,?,?,?,?,?) RETURNING id").bind(required(payload,"nickname"),required(payload,"bankName"),String(payload.currency??"RUB"),accountLast4,accountNumber,String(payload.accountName??""),String(payload.bankAddress??""),String(payload.swiftCode??""),String(payload.ifscCode??""),String(payload.accountType??""),Number(payload.balance??0),payload.isDefault?1:0).first<{id:number}>();
      if(bank)await audit(actor,"created","bank account",bank.id,`${actor.displayName} added bank account ${payload.nickname}`,{bankName:payload.bankName,currency:payload.currency,accountType:payload.accountType,isDefault:Boolean(payload.isDefault)});
      ...
      const orderIdRaw=String(payload.orderId??"").trim();const orderId=orderIdRaw?Number(orderIdRaw):null;
      let clientId=Number(payload.clientId);
      let order:Record<string,unknown>|null=null;
      if(orderId){
        order=await db.prepare("SELECT id,client_id,description,sale_price,sale_currency,purchase_price,purchase_currency FROM orders WHERE id=?").bind(orderId).first<Record<string,unknown>>();
        if(!order)throw new Error("Choose a valid order");
        clientId=Number(order.client_id);
      }else if(!clientId)throw new Error("Choose a client or supplier");
      const client=await db.prepare("SELECT id FROM clients WHERE id=?").bind(clientId).first<{id:number}>();if(!client)throw new Error("Choose a valid client or supplier");
      await assertOperatorClientAccess(db,actor,clientId);
      const bankAccountId=Number(payload.bankAccountId);if(!bankAccountId)throw new Error("Choose a company bank account");
      const bankAccount=await db.prepare("SELECT id FROM bank_accounts WHERE id=?").bind(bankAccountId).first<{id:number}>();if(!bankAccount)throw new Error("Choose a valid company bank account");
